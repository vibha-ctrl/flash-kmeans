...
_TUNE_CONFIGS = [
    triton.Config({"BLOCK_N": 32, "BLOCK_K": BK}, num_stages=ns, num_warps=1)
...
def euclid_assign_splitq(x: torch.Tensor, centroids: torch.Tensor, x_sq: torch.Tensor, out: torch.Tensor = None, c_sq: torch.Tensor = None,
...
def cosine_assign_splitq(x: torch.Tensor, centroids: torch.Tensor, out: torch.Tensor = None,
                         *, BLOCK_N: int = 128, BLOCK_K: int = 128) -> torch.Tensor:
...
if __name__ == "__main__":
    ...
    tri_ids = euclid_assign_splitq(x, cent, x_sq, out)
    ...
    tri_ids_cos = cosine_assign_splitq(x, cent, out)
    ...
    for _ in range(repeats):
        euclid_assign_splitq(x, cent, x_sq, out)
    ...
    start.record()
    for _ in range(repeats):
        cosine_assign_splitq(x, cent, out)
    end.record(); torch.cuda.synchronize()
